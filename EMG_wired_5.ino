...
uint8_t ledScale(uint8_t pin, int refMaxBit);
...
void loop() {
  ...
  if (buttonCounter == 1) {
    ...
    uint8_t ledLevel = ledScale(sensorPin, refMaxBit);  //Scales LED-bar to certain voltage width (0-3.3 V) and returns LED-Level

    //Sending avrg value bits vie uart
    Serial1.write(ledLevel & 0xFF);  //Sending 8-bit package over UART-interface
    ...
    if (millis() - lastPrintTime > 200) {  //Output control, prevent Serial.monitor spaming
      ...
      Serial.print(" V |--Output UART--| LED-Level: ");
  ...
  if (buttonCounter == 2) {
    ...
    uint8_t ledLevel = (buttonStateT == LOW) ? 10 : 0;  //Convert buttonStateT from bool to uint8_t, (condition)? if true: if false
    Serial1.write(ledLevel & 0xFF);                     //Sending 8-bit package over UART-interface
    ...
    if (millis() - lastPrintTime > 200) {  //Spam-controll
      ...
      Serial.print(" |--Output UART--| LED-Level: ");
      Serial.println(ledLevel);
      lastPrintTime = millis();
    }
  }
  ...
  //Potentiometer
  if (buttonCounter == 3) {
    uint8_t ledLevel = ledScale(potiPin, refMaxBit);
...
//LedScaling to Bits
uint8_t ledScale(uint8_t pin, int refMaxBit) {  //Scaling bits of a certain pin (ADC) to the 10 LED-levels of the LED-bar
  uint16_t val = analogRead(pin);
  uint8_t ledLevel = map(val, 0, refMaxBit, 0, 10);
